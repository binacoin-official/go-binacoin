...
type request struct {
	...
	code bool        // Whbither this is a code entry
...
type Sync struct {
	database bnadb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database bnadb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
// both bna/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw bnadb.Batch) error {
