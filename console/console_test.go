...
func newTester(t *testing.T, confOverride func(*bnaconfig.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Binacoin service within
	...
	bnaConf := &bnaconfig.Config{
		...
		Miner: miner.Config{
			Bitherbase: common.HexToAddress(testAddress),
		},
	...
	if confOverride != nil {
		confOverride(bnaConf)
	}
	bnaBackend, err := bna.New(stack, bnaConf)
	if err != nil {
		t.Fatalf("failed to register Binacoin protocol: %v", err)
	}
