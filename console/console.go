...
import (
	...
	"github.com/binacoin-official/go-binacoin/internal/jsre"
	"github.com/binacoin-official/go-binacoin/internal/jsre/deps"
	"github.com/binacoin-official/go-binacoin/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Binacoin requests through
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"bna": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines bna.getBalance(bna.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gbina instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gbina JavaScript console!\n\n"

	// Print some generic Gbina metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + bna.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + bna.blockNumber + " (" + new Date(1000 * bna.getBlock(bna.blockNumber).timestamp) + ")\n";
		try {
