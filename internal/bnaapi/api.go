...
// PublicBinacoinAPI provides an API to access Binacoin related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBinacoinAPI struct {
	b Backend
}

// NewPublicBinacoinAPI creates a new Binacoin protocol API.
func NewPublicBinacoinAPI(b Backend) *PublicBinacoinAPI {
	return &PublicBinacoinAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicBinacoinAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicBinacoinAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicBinacoinAPI) FeeHistory(ctx context.Context, blockCount int, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *PublicBinacoinAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Binacoin ECDSA signature for:
// keccack256("\x19Binacoin Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/binacoin-official/go-binacoin/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Binacoin Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/binacoin-official/go-binacoin/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Binacoin signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gbina.
...
// PublicBlockChainAPI provides an API to access the Binacoin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Binacoin blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current binacoin chain config.
...
// See: https://github.com/binacoin-official/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/binacoin-official/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Binacoin APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Binacoin APIs exposed over the private
// debugging endpoint.
...
// Version returns the current binacoin protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeBna := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Bither)))
	feeFloat, _ := feeBna.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f bither) exceeds the configured cap (%.2f bither)", feeFloat, cap)
	}
	return nil
}
