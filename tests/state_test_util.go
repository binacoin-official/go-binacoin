...
// See https://github.com/binacoin-official/EIPs/issues/176 for the test format specification.
...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool) (*snapshot.Tree, *state.StateDB, common.Hash, error) {
	...
	if config.IsLondon(new(big.Int)) {
		...
		if baseFee == nil {
			// Retestbna uses `0x10` for genesis baseFee. Therefore, it defaults to
...
func MakePreState(db bnadb.Database, accounts core.GenesisAlloc, snapshotter bool) (*snapshot.Tree, *state.StateDB) {
...
func (tx *stTransaction) toMessage(ps stPostState, baseFee *big.Int) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/binacoin-official/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
