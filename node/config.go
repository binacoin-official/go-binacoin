...
import (
	...
	"github.com/binacoin-official/go-binacoin/accounts/external"
	"github.com/binacoin-official/go-binacoin/accounts/keystore"
	"github.com/binacoin-official/go-binacoin/accounts/scwallet"
	"github.com/binacoin-official/go-binacoin/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gbina is "gbina". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gbina", keep that.
	if name == "gbina" || name == "gbina-testnet" {
		name = "Gbina"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gbina" instances.
var isOldGbinaResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gbina 1.4 are used if they exist.
	if warn, isOld := isOldGbinaResource[path]; isOld {
		oldpath := ""
		if c.name() == "gbina" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGbinaResourceWarning, "Using deprecated resource file %s, please move this file to the 'gbina' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-binacoin-keystore")
		ephemeral = keydir
	}
