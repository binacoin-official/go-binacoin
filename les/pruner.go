...
func newPruner(db bnadb.Database, indexers ...*core.ChainIndexer) *pruner {
	pruner := &pruner{
		db:       db,
		indexers: indexers,
...
// historical chain data. Notably, whenever Gbina restarts, it will iterate
...
func (p *pruner) loop() {
	...
	// Note, if some indexers don't support pruning(e.g. bna.BloomIndexer),
