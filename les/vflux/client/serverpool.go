...
import (
	...
	"github.com/binacoin-official/go-binacoin/p2p/nodestate"
...
func NewServerPool(db bnadb.KeyValueStore, dbKey []byte, mixTimeout time.Duration, query QueryFunc, clock mclock.Clock, trustedURLs []string, requestList []RequestInfo) (*ServerPool, enode.Iterator) {
	s := &ServerPool{
		db:           db,
		clock:        clock,
