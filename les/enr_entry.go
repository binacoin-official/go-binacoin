...
// ethEntry is the "bna" ENR entry. This is redeclared here to avoid depending on package bna.
type ethEntry struct {
	ForkID forkid.ID
	Tail   []rlp.RawValue `rlp:"tail"`
}

func (ethEntry) ENRKey() string { return "bna" }
...
func (bna *LightBinacoin) setupDiscovery() (enode.Iterator, error) {
	...
	if len(bna.config.EthDiscoveryURLs) != 0 {
		client := dnsdisc.NewClient(dnsdisc.Config{})
		dns, err := client.NewIterator(bna.config.EthDiscoveryURLs...)
		if err != nil {
			return nil, err
		}
		it.AddSource(dns)
