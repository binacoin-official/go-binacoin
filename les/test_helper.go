...
import (
	...
	"github.com/binacoin-official/go-binacoin/accounts/abi/bind/backends"
	...
	"github.com/binacoin-official/go-binacoin/contracts/checkpointoracle/contract"
	...
	"github.com/binacoin-official/go-binacoin/les/checkpointoracle"
	"github.com/binacoin-official/go-binacoin/les/flowcontrol"
	vfs "github.com/binacoin-official/go-binacoin/les/vflux/server"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some bither to user1
		...
		case 1:
			...
			// bankUser transfers more bither to user1
		...
		case 2:
			...
			// bankUser transfer some bither to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db bnadb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db bnadb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) (*clientHandler, func()) {
	var (
	...
	client := &LightBinacoin{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &bnaconfig.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db bnadb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend, func()) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
