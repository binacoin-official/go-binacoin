...
type LesServer struct {
	...
	archiveMode bool // Flag whether the binacoin node runs in archive mode.
...
func NewLesServer(node *node.Node, e bnaBackend, config *bnaconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "bna/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "Binacoin light client service")
