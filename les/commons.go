...
// NodeInfo represents a short summary of the Binacoin sub-protocol metadata
...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, bnaconfig *bnaconfig.Config) *checkpointoracle.CheckpointOracle {
	config := bnaconfig.CheckpointOracle
	if config == nil {
	...
	client := bnaclient.NewClient(rpcClient)
	oracle.Start(client)
