...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *LightBinacoin) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer) error {
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light Binacoin peer registration failed", "err", err)
		return err
	}
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, bna.BNA65, pc)
}
