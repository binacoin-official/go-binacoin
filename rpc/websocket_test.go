...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.BnaSubscribe(ctx, resultChan, "foo")
	if err != nil {
	...
	// This is important for reproducing https://github.com/binacoin-official/go-binacoin/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the bna_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","method":"bna_subscription","params":{"subscription":"0x00","result":1}}`
	)
