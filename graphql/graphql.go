...
// Package graphql provides a GraphQL interface to Binacoin node data.
package graphql
...
// Account represents an Binacoin account at a particular block.
type Account struct {
	backend       bnaapi.Backend
...
// Transaction represents an Binacoin transaction.
...
// Block represents an Binacoin block.
...
// `Log` objects.
func runFilter(ctx context.Context, be bnaapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From                 *common.Address // The Binacoin address the call is from.
	To                   *common.Address // The Binacoin address the call is to.
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := bnaapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, 5*time.Second, b.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := bnaapi.DoEstimateGas(ctx, b.backend, args.Data, *b.numberOrHash, b.backend.RPCGasCap())
	return Long(gas), err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := bnaapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, 5*time.Second, p.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	gas, err := bnaapi.DoEstimateGas(ctx, p.backend, args.Data, pendingBlockNr, p.backend.RPCGasCap())
	return Long(gas), err
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := bnaapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
...
type SyncState struct {
	progress binacoin.SyncProgress
}
