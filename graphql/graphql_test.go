...
func createGQLService(t *testing.T, stack *node.Node) {
	...
	bnaBackend, err := bna.New(stack, bnaConf)
	if err != nil {
		t.Fatalf("could not create bna backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AllEthashProtocolChanges, bnaBackend.BlockChain().Genesis(),
		ethash.NewFaker(), bnaBackend.ChainDb(), 10, func(i int, gen *core.BlockGen) {})
	_, err = bnaBackend.BlockChain().InsertChain(chain)
	if err != nil {
		t.Fatalf("could not create import blocks: %v", err)
	}
	// create gql service
	err = New(stack, bnaBackend.APIBackend, []string{}, []string{})
	if err != nil {
...
func createGQLServiceWithTransactions(t *testing.T, stack *node.Node) {
	...
	signer := types.LatestSigner(bnaConf.Genesis.Config)
	...
	envelopTx, _ := types.SignNewTx(key, signer, &types.AccessListTx{
		ChainID:  bnaConf.Genesis.Config.ChainID,
		Nonce:    uint64(1),
		To:       &dad,
		Gas:      30000,
		GasPrice: big.NewInt(params.InitialBaseFee),
	...
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AllEthashProtocolChanges, bnaBackend.BlockChain().Genesis(),
		ethash.NewFaker(), bnaBackend.ChainDb(), 1, func(i int, b *core.BlockGen) {
