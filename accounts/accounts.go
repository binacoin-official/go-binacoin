...
// Package accounts implements high level Binacoin account management.
package accounts
...
// Account represents an Binacoin account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Binacoin account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain binacoin.ChainStateReader)
...
//   keccak256("\x19Binacoin Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Binacoin Signed Message:\n%d%s", len(data), string(data))
