...
import (
	...
	"github.com/binacoin-official/go-binacoin/accounts/usbwallet/trezor"
...
// or whether the Binacoin app was not started on it.
...
// Binacoin address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.BinacoinAddress)
	if _, err := w.trezorExchange(&trezor.BinacoinGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.BinacoinTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BinacoinTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
