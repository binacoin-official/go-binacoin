...
func (h *testEthHandler) TxPool() bna.TxPool                   { panic("no backing tx pool") }
func (h *testEthHandler) AcceptTxs() bool                      { return true }
func (h *testEthHandler) RunPeer(*bna.Peer, bna.Handler) error { panic("not used in tests") }
func (h *testEthHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testEthHandler) Handle(peer *bna.Peer, packet bna.Packet) error {
...
func TestForkIDSplit65(t *testing.T) { testForkIDSplit(t, bna.ETH65) }
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, bna.ETH66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	go func(errc chan error) {
		errc <- ethNoFork.runEthPeer(peerProFork, func(peer *bna.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- ethProFork.runEthPeer(peerNoFork, func(peer *bna.Peer) error { return nil })
	}(errc)
...
func TestRecvTransactions65(t *testing.T) { testRecvTransactions(t, bna.ETH65) }
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, bna.ETH66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runEthPeer(sink, func(peer *bna.Peer) error {
		return bna.Handle((*ethHandler)(handler.handler), peer)
	})
...
func TestSendTransactions65(t *testing.T) { testSendTransactions(t, bna.ETH65) }
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, bna.ETH66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runEthPeer(src, func(peer *bna.Peer) error {
		return bna.Handle((*ethHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
...
func TestTransactionPropagation65(t *testing.T) { testTransactionPropagation(t, bna.ETH65) }
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, bna.ETH66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		go source.handler.runEthPeer(sourcePeer, func(peer *bna.Peer) error {
			return bna.Handle((*ethHandler)(source.handler), peer)
		})
		go sink.handler.runEthPeer(sinkPeer, func(peer *bna.Peer) error {
			return bna.Handle((*ethHandler)(sink.handler), peer)
		})
	}
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := bna.NewPeer(bna.ETH65, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := bna.NewPeer(bna.ETH65, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runEthPeer(local, func(peer *bna.Peer) error {
			return bna.Handle((*ethHandler)(handler.handler), peer)
		})
	}()
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := bna.NewPeer(bna.ETH65, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := bna.NewPeer(bna.ETH65, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock65(t *testing.T) { testBroadcastMalformedBlock(t, bna.ETH65) }
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, bna.ETH66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	go source.handler.runEthPeer(src, func(peer *bna.Peer) error {
		return bna.Handle((*ethHandler)(source.handler), peer)
	})
