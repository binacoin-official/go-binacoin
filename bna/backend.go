...
func New(stack *node.Node, config *ethconfig.Config) (*Binacoin, error) {
	...
	bna.APIBackend = &EthAPIBackend{stack.Config().ExtRPCEnabled(), stack.Config().AllowUnprotectedTxs, bna, nil}
	...
	bna.ethDialCandidates, err = dnsclient.NewIterator(bna.config.EthDiscoveryURLs...)
	if err != nil {
		return nil, err
	}
	...
	bna.netRPCService = ethapi.NewPublicNetAPI(bna.p2pServer, config.NetworkId)
...
func (s *Binacoin) Etherbase() (eb common.Address, err error) {
	s.lock.RLock()
...
func (s *Binacoin) SetEtherbase(etherbase common.Address) {
	s.lock.Lock()
	s.etherbase = etherbase
	s.lock.Unlock()
...
func (s *Binacoin) ChainDb() ethdb.Database            { return s.chainDb }
...
func (s *Binacoin) Protocols() []p2p.Protocol {
	protos := bna.MakeProtocols((*ethHandler)(s.handler), s.networkID, s.ethDialCandidates)
