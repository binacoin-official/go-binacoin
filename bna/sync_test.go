...
func TestFastSyncDisabling65(t *testing.T) { testFastSyncDisabling(t, bna.ETH65) }
func TestFastSyncDisabling66(t *testing.T) { testFastSyncDisabling(t, bna.ETH66) }
...
func testFastSyncDisabling(t *testing.T, protocol uint) {
	...
	go empty.handler.runEthPeer(emptyPeer, func(peer *bna.Peer) error {
		return bna.Handle((*ethHandler)(empty.handler), peer)
	})
	go full.handler.runEthPeer(fullPeer, func(peer *bna.Peer) error {
		return bna.Handle((*ethHandler)(full.handler), peer)
	})
