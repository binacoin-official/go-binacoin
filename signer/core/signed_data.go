...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useBinacoinV = true // Default to use V = 27 or 28, the legacy Binacoin format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useBinacoinV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the Binacoin PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useBinacoinV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Binacoin ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Binacoin Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useBinacoinV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	//
	// Note, this function is compatible with bna_sign and personal_sign. As such it recovers
	// the address of:
	// hash = keccak256("\x19${byteVersion}Binacoin Signed Message:\n${message length}${message}")
