...
// testBnaHandler is a mock event handler to listen for inbound network requests
// on the `bna` protocol and convert them into a more easily testable form.
type testBnaHandler struct {
	blockBroadcasts event.Feed
	txAnnounces     event.Feed
	txBroadcasts    event.Feed
}

func (h *testBnaHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testBnaHandler) StateBloom() *trie.SyncBloom          { panic("no backing state bloom") }
func (h *testBnaHandler) TxPool() bna.TxPool                   { panic("no backing tx pool") }
func (h *testBnaHandler) AcceptTxs() bool                      { return true }
func (h *testBnaHandler) RunPeer(*bna.Peer, bna.Handler) error { panic("not used in tests") }
func (h *testBnaHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testBnaHandler) Handle(peer *bna.Peer, packet bna.Packet) error {
	switch packet := packet.(type) {
	case *bna.NewBlockPacket:
		h.blockBroadcasts.Send(packet.Block)
		return nil

	case *bna.NewPooledTransactionHashesPacket:
		h.txAnnounces.Send(([]common.Hash)(*packet))
		return nil

	case *bna.TransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	case *bna.PooledTransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	default:
		panic(fmt.Sprintf("unexpected bna packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit65(t *testing.T) { testForkIDSplit(t, bna.BNA65) }
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, bna.BNA66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		bnaNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		bnaProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Network:    1,
	...
	bnaNoFork.Start(1000)
	bnaProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer bnaNoFork.Stop()
	defer bnaProFork.Stop()
	...
	peerNoFork := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- bnaNoFork.runBnaPeer(peerProFork, func(peer *bna.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- bnaProFork.runBnaPeer(peerNoFork, func(peer *bna.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = bna.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = bna.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions65(t *testing.T) { testRecvTransactions(t, bna.BNA65) }
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, bna.BNA66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runBnaPeer(sink, func(peer *bna.Peer) error {
		return bna.Handle((*bnaHandler)(handler.handler), peer)
	})
...
func TestSendTransactions65(t *testing.T) { testSendTransactions(t, bna.BNA65) }
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, bna.BNA66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runBnaPeer(src, func(peer *bna.Peer) error {
		return bna.Handle((*bnaHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 65, 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post bna/65")
			}
...
func TestTransactionPropagation65(t *testing.T) { testTransactionPropagation(t, bna.BNA65) }
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, bna.BNA66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runBnaPeer(sourcePeer, func(peer *bna.Peer) error {
			return bna.Handle((*bnaHandler)(source.handler), peer)
		})
		go sink.handler.runBnaPeer(sinkPeer, func(peer *bna.Peer) error {
			return bna.Handle((*bnaHandler)(sink.handler), peer)
		})
	}
...
// Tests that post bna protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := bna.NewPeer(bna.BNA65, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := bna.NewPeer(bna.BNA65, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runBnaPeer(local, func(peer *bna.Peer) error {
			return bna.Handle((*bnaHandler)(handler.handler), peer)
		})
	}()
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testBnaHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testBnaHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := bna.NewPeer(bna.BNA65, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := bna.NewPeer(bna.BNA65, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock65(t *testing.T) { testBroadcastMalformedBlock(t, bna.BNA65) }
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, bna.BNA66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := bna.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runBnaPeer(src, func(peer *bna.Peer) error {
		return bna.Handle((*bnaHandler)(source.handler), peer)
	})
