...
import (
	...
	chainParams "github.com/binacoin-official/go-binacoin/params"
	"github.com/binacoin-official/go-binacoin/rpc"
	"github.com/binacoin-official/go-binacoin/trie"
)

// Register adds catalyst APIs to the node.
func Register(stack *node.Node, backend *bna.Binacoin) error {
...
func newConsensusAPI(bna *bna.Binacoin) *consensusAPI {
	return &consensusAPI{bna: bna}
}
...
func (api *consensusAPI) makeEnv(parent *types.Block, header *types.Header) (*blockExecutionEnv, error) {
	state, err := api.bna.BlockChain().StateAt(parent.Root())
	if err != nil {
		return nil, err
	}
	env := &blockExecutionEnv{
		chain:   api.bna.BlockChain(),
		state:   state,
		header:  header,
...
func (api *consensusAPI) AssembleBlock(params assembleBlockParams) (*executableData, error) {
	...
	coinbase, err := api.bna.Bitherbase()
	if err != nil {
		return nil, err
	}
	num := parent.Number()
	...
	if config := api.bna.BlockChain().Config(); config.IsLondon(header.Number) {
		header.BaseFee = misc.CalcBaseFee(config, parent.Header())
	}
	err = api.bna.Engine().Prepare(bc, header)
	if err != nil {
		return nil, err
	}
	...
	// Create the block.
	block, err := api.bna.Engine().FinalizeAndAssemble(bc, header, env.state, transactions, nil /* uncles */, env.receipts)
	if err != nil {
		return nil, err
	}
...
// NewBlock creates an Bna1 block, inserts it in the chain, and either returns true,
...
func (api *consensusAPI) NewBlock(params executableData) (*newBlockResponse, error) {
	parent := api.bna.BlockChain().GetBlockByHash(params.ParentHash)
	if parent == nil {
		return &newBlockResponse{false}, fmt.Errorf("could not find parent %x", params.ParentHash)
	}
	block, err := insertBlockParamsToBlock(api.bna.BlockChain().Config(), parent.Header(), params)
	if err != nil {
		return nil, err
	}
	_, err = api.bna.BlockChain().InsertChainWithoutSealVerification(block)
