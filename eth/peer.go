...
// bnaPeerInfo represents a short summary of the `bna` sub-protocol metadata known
// about a connected peer.
type bnaPeerInfo struct {
	Version    uint     `json:"version"`    // Binacoin protocol version negotiated
	Difficulty *big.Int `json:"difficulty"` // Total difficulty of the peer's blockchain
	Head       string   `json:"head"`       // Hex hash of the peer's best owned block
}

// bnaPeer is a wrapper around bna.Peer to maintain a few extra metadata.
type bnaPeer struct {
	*bna.Peer
	snapExt *snapPeer // Satellite `snap` connection

	syncDrop *time.Timer   // Connection dropper if `bna` sync progress isn't validated in time
	snapWait chan struct{} // Notification channel for snap connections
	lock     sync.RWMutex  // Mutex protecting the internal fields
}

// info gathers and returns some `bna` protocol metadata known about a peer.
func (p *bnaPeer) info() *bnaPeerInfo {
	hash, td := p.Head()
