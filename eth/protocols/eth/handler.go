...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `bna` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `bna`.
...
// NodeInfo represents a short summary of the `bna` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Binacoin network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
...
// nodeInfo retrieves some `bna` protocol metadata about the running host node.
...
// Handle is invoked whenever an `bna` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `bna`", "err", err)
			return err
		}
	}
}
...
var bna65 = map[uint64]msgHandler{
...
var bna66 = map[uint64]msgHandler{
	NewBlockHashesMsg:             handleNewBlockhashes,
	NewBlockMsg:                   handleNewBlock,
	TransactionsMsg:               handleTransactions,
	NewPooledTransactionHashesMsg: handleNewPooledTransactionHashes,
	// bna66 messages with request-id
