...
// enrEntry is the ENR entry which advertises `bna` protocol on the discovery.
...
// StartENRUpdater starts the `bna` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `bna` ENR entry based on the current state of the chain.
