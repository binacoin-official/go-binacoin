...
// ProtocolName is the official short name of the `bna` protocol used during
// devp2p capability negotiation.
const ProtocolName = "bna"

// ProtocolVersions are the supported versions of the `bna` protocol (first
// is primary).
var ProtocolVersions = []uint{BNA66, BNA65}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{BNA66: 17, BNA65: 17}

// maxMessageSize is the maximum cap on the size of a protocol message.
const maxMessageSize = 10 * 1024 * 1024

const (
	// Protocol messages in bna/64
	StatusMsg          = 0x00
	...
	// Protocol messages overloaded in bna/65
...
// Packet represents a p2p message in the `bna` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// StatusPacket is the network packet for the status message for bna/64 and later.
...
// GetBlockHeadersPacket represents a block header query over bna/66
...
// BlockHeadersPacket represents a block header response over bna/66.
...
// GetBlockBodiesPacket represents a block body query over bna/66.
...
// BlockBodiesPacket is the network packet for block content distribution over bna/66.
...
// BlockBodiesRLPPacket66 is the BlockBodiesRLPPacket over bna/66
...
// GetNodeDataPacket represents a trie node data query over bna/66.
...
// NodeDataPacket is the network packet for trie node data distribution over bna/66.
...
// GetReceiptsPacket represents a block receipts query over bna/66.
...
// ReceiptsPacket is the network packet for block receipts distribution over bna/66.
...
// ReceiptsPacket66 is the bna-66 version of ReceiptsRLPPacket
...
// PooledTransactionsPacket is the network packet for transaction distribution over bna/66.
...
// PooledTransactionsRLPPacket66 is the bna/66 form of PooledTransactionsRLPPacket
