...
import (
	...
	"github.com/binacoin-official/go-binacoin/p2p/msgrate"
...
type accountResponse struct {
	...
	cont bool // Whbither the account range has a continuation
}
...
type storageResponse struct {
	...
	cont bool // Whbither the last storage range has a continuation
}
...
type accountTask struct {
	...
	genBatch bnadb.Batch     // Batch used by the node generator
...
// Syncer is an Binacoin account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db bnadb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        bnadb.Batch        // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the Binacoin state over the
// snap protocol.
func NewSyncer(db bnadb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Whbither sync completed or not, disregard any future packets
	defer func() {
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = bnadb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = bnadb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > bnadb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > bnadb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Whbither or not the response is valid, we can mark the peer as idle and
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > bnadb.IdealBatchSize {
