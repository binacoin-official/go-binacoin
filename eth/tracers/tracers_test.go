...
import (
	...
	"github.com/binacoin-official/go-binacoin/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gbina console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = bna.getBlock(bna.getTransaction(tx).blockHash);
  var genesis = bna.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.bna.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  bna.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
