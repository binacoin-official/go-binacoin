...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.Bither)},
		accounts[1].addr: {Balance: big.NewInt(params.Bither)},
		accounts[2].addr: {Balance: big.NewInt(params.Bither)},
	}}
	genBlocks := 10
	...
	var testSuite = []struct {
		...
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &bnaapi.ExecutionResult{
				Gas:         params.TxGas,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []bnaapi.StructLogRes{},
			},
		},
...
func TestOverridenTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Succcessful call with state overriding
		{
			...
			config: &TraceCallConfig{
				Tracer: &tracer,
				StateOverrides: &bnaapi.StateOverride{
					randomAccounts[0].addr: bnaapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Bither)))},
				},
			},
			expectErr: nil,
			expect: &callTrace{
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				Tracer: &tracer,
				StateOverrides: &bnaapi.StateOverride{
					randomAccounts[2].addr: bnaapi.OverrideAccount{
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &bnaapi.ExecutionResult{
		Gas:         params.TxGas,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []bnaapi.StructLogRes{},
