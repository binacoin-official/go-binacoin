...
var (
	...
	// errSnapWithoutBna is returned if a peer attempts to connect only on the
	// snap protocol without advertizing the bna main protocol.
	errSnapWithoutBna = errors.New("peer connected on snap without compatible bna support")
)

// peerSet represents the collection of active peers currently participating in
// the `bna` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*bnaPeer // Peers connected on the `bna` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `bna` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `bna`
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*bnaPeer),
		snapWait: make(map[string]chan *snap.Peer),
		snapPend: make(map[string]*snap.Peer),
	}
}

// registerSnapExtension unblocks an already connected `bna` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `bna` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `bna` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `bna`
	if !peer.RunningCap(bna.ProtocolName, bna.ProtocolVersions) {
		return errSnapWithoutBna
	}
	...
	// Inject the peer into an `bna` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *bna.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `bna` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *bna.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *bnaPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*bnaPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*bnaPeer, 0, len(ps.peers))
...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*bnaPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
// len returns if the current number of `bna` peers in the set. Since the `snap`
// peers are tied to the existence of an `bna` connection, that will always be a
// subset of `bna`.
...
// difficulty.
func (ps *peerSet) peerWithHighestTD() *bna.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
