...
// bnaHandler implements the bna.Backend interface to handle the various network
// packets that are sent as replies or broadcasts.
type bnaHandler handler

func (h *bnaHandler) Chain() *core.BlockChain     { return h.chain }
func (h *bnaHandler) StateBloom() *trie.SyncBloom { return h.stateBloom }
func (h *bnaHandler) TxPool() bna.TxPool          { return h.txpool }

// RunPeer is invoked when a peer joins on the `bna` protocol.
func (h *bnaHandler) RunPeer(peer *bna.Peer, hand bna.Handler) error {
	return (*handler)(h).runBnaPeer(peer, hand)
}

// PeerInfo retrieves all known `bna` information about a peer.
func (h *bnaHandler) PeerInfo(id enode.ID) interface{} {
...
func (h *bnaHandler) Handle(peer *bna.Peer, packet bna.Packet) error {
	...
	default:
		return fmt.Errorf("unexpected bna packet type: %T", packet)
	}
}

// handleHeaders is invoked from a peer's message handler when it transmits a batch
// of headers for the local node to process.
func (h *bnaHandler) handleHeaders(peer *bna.Peer, headers []*types.Header) error {
...
func (h *bnaHandler) handleBodies(peer *bna.Peer, txs [][]*types.Transaction, uncles [][]*types.Header) error {
...
func (h *bnaHandler) handleBlockAnnounces(peer *bna.Peer, hashes []common.Hash, numbers []uint64) error {
...
func (h *bnaHandler) handleBlockBroadcast(peer *bna.Peer, block *types.Block, td *big.Int) error {
