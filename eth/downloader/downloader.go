...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [bna/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    bnadb.Database  // Database to state sync into (and deduplicate via)
	...
	snapSync       bool         // Whbither to run state sync over the snap protocol
...
func New(checkpoint uint64, stateDb bnadb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() binacoin.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < bna.BNA65 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, bna.BNA65)
	}
	mode := d.getMode()

	log.Debug("Synchronising with the network", "peer", p.id, "bna", p.version, "head", hash, "td", td, "mode", mode)
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whbither the next request is pivot verification
