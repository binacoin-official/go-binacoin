...
// PublicBinacoinAPI provides an API to access Binacoin full node-related
// information.
...
func (api *PrivateMinerAPI) SetBitherbase(bitherbase common.Address) bool {
	api.e.SetBitherbase(bitherbase)
	return true
}
...
// PrivateAdminAPI is the collection of Binacoin full node-related APIs
...
// admin methods of the Binacoin service.
func NewPrivateAdminAPI(bna *Binacoin) *PrivateAdminAPI {
	return &PrivateAdminAPI{bna: bna}
}
...
func (api *PrivateAdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.bna.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.bna.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.bna.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.bna.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.bna.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Binacoin full node APIs exposed
...
// related public debug methods of the Binacoin service.
func NewPublicDebugAPI(bna *Binacoin) *PublicDebugAPI {
	return &PublicDebugAPI{bna: bna}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.bna.miner.Pending()
		return stateDb.RawDump(opts), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.bna.blockchain.CurrentBlock()
	} else {
		block = api.bna.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.bna.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Binacoin full node APIs exposed over
...
// private debug methods of the Binacoin service.
func NewPrivateDebugAPI(bna *Binacoin) *PrivateDebugAPI {
	return &PrivateDebugAPI{bna: bna}
}

// Preimage is a debug API function that returns the preimage for a sha3 hash, if known.
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.bna.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		err     error
		blocks  = rawdb.ReadAllBadBlocks(api.bna.chainDb)
	...
	for _, block := range blocks {
		...
		if blockJSON, err = bnaapi.RPCMarshalBlock(block, true, true); err != nil {
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			if number == rpc.LatestBlockNumber {
				...
				block = api.bna.blockchain.GetBlockByNumber(uint64(number))
			}
			if block == nil {
		...
		block := api.bna.blockchain.GetBlockByHash(hash)
		if block == nil {
...
func (api *PrivateDebugAPI) StorageRangeAt(blockHash common.Hash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	// Retrieve the block
	block := api.bna.blockchain.GetBlockByHash(blockHash)
	if block == nil {
		return StorageRangeResult{}, fmt.Errorf("block %#x not found", blockHash)
	}
	_, _, statedb, err := api.bna.stateAtTransaction(block, txIndex, 0)
	if err != nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.bna.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.bna.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.bna.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.bna.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.bna.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.bna.BlockChain().StateCache().TrieDB()
