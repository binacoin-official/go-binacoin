...
func (s *Suite) sendSuccessfulTxs(t *utesting.T, isBna66 bool) error {
	...
	for i, tx := range tests {
		...
		if err := sendSuccessfulTx(s, tx, prevTx, isBna66); err != nil {
...
func sendSuccessfulTx(s *Suite, tx *types.Transaction, prevTx *types.Transaction, isBna66 bool) error {
...
func (s *Suite) sendMaliciousTxs(t *utesting.T, isBna66 bool) error {
	...
	for i, tx := range badTxs {
		...
		if err = sendMaliciousTx(s, tx, isBna66); err != nil {
...
func sendMaliciousTx(s *Suite, tx *types.Transaction, isBna66 bool) error {
	// setup connection
	var (
		conn *Conn
		err  error
