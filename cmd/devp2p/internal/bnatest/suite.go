...
// NewSuite creates and returns a new bna-test suite that can
...
func (s *Suite) AllBnaTests() []utesting.Test {
...
func (s *Suite) BnaTests() []utesting.Test {
	return []utesting.Test{
		{Name: "TestStatus", Fn: s.TestStatus},
		{Name: "TestGetBlockHeaders", Fn: s.TestGetBlockHeaders},
		{Name: "TestGetBlockBodies", Fn: s.TestGetBlockBodies},
...
func (s *Suite) Bna66Tests() []utesting.Test {
	return []utesting.Test{
		// only proceed with bna66 test suite if node supports bna 66 protocol
		{Name: "TestStatus66", Fn: s.TestStatus66},
		{Name: "TestGetBlockHeaders66", Fn: s.TestGetBlockHeaders66},
		{Name: "TestSimultaneousRequests66", Fn: s.TestSimultaneousRequests66},
		{Name: "TestSameRequestID66", Fn: s.TestSameRequestID66},
...
var (
	bna66 = true  // indicates whether suite should negotiate bna66 connection
	bna65 = false // indicates whether suite should negotiate bna65 connection or below.
)
...
// a status message with it on the bna66 protocol.
...
// an bna66 `GetBlockHeaders` request and that the response is accurate.
func (s *Suite) TestGetBlockHeaders66(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, bna66, 33)
	if err != nil {
...
func (s *Suite) TestSimultaneousRequests66(t *utesting.T) {
	...
	// create two requests
	req1 := &bna.GetBlockHeadersPacket66{
		RequestId: uint64(111),
		GetBlockHeadersPacket: &bna.GetBlockHeadersPacket{
			Origin: bna.HashOrNumber{
				Hash: s.chain.blocks[1].Hash(),
			},
			Amount:  2,
	...
	req2 := &bna.GetBlockHeadersPacket66{
...
func (s *Suite) TestSameRequestID66(t *utesting.T) {
	...
	// create requests
	reqID := uint64(1234)
	request1 := &bna.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &bna.GetBlockHeadersPacket{
			Origin: bna.HashOrNumber{
				Number: 1,
	...
	request2 := &bna.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &bna.GetBlockHeadersPacket{
			Origin: bna.HashOrNumber{
				Number: 33,
...
func (s *Suite) TestZeroRequestID66(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, bna66, 0)
	if err != nil {
...
func (s *Suite) TestGetBlockBodies66(t *utesting.T) {
	...
	// create block bodies request
	req := &bna.GetBlockBodiesPacket66{
		RequestId: uint64(55),
		GetBlockBodiesPacket: bna.GetBlockBodiesPacket{
			s.chain.blocks[54].Hash(),
			s.chain.blocks[75].Hash(),
		},
	}
...
func (s *Suite) TestBlockHashAnnounce(t *utesting.T) {
	if err := s.hashAnnounce(bna65); err != nil {
...
func (s *Suite) TestBlockHashAnnounce66(t *utesting.T) {
	if err := s.hashAnnounce(bna66); err != nil {
		t.Fatalf("block hash announcement failed: %v", err)
	}
}
...
func (s *Suite) TestMaliciousHandshake(t *utesting.T) {
	if err := s.maliciousHandshakes(t, bna65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousHandshake66(t *utesting.T) {
	if err := s.maliciousHandshakes(t, bna66); err != nil {
		t.Fatal(err)
	}
}
...
// difficulty over the bna66 protocol.
...
func (s *Suite) TestTransaction(t *utesting.T) {
	if err := s.sendSuccessfulTxs(t, bna65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestTransaction66(t *utesting.T) {
	if err := s.sendSuccessfulTxs(t, bna66); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousTx(t *utesting.T) {
	if err := s.sendMaliciousTxs(t, bna65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousTx66(t *utesting.T) {
	if err := s.sendMaliciousTxs(t, bna66); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestLargeTxRequest66(t *utesting.T) {
	...
	getTxReq := &bna.GetPooledTransactionsPacket66{
		RequestId:                   1234,
