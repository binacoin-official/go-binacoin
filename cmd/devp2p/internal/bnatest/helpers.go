...
// Is_66 checks if the node supports the bna66 protocol version,
...
// returning the created Conn with additional bna66 capabilities if
// successful
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "bna", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		c.negotiateBnaProtocol(msg.Caps)
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected bna protocol version")
		}
		return nil
	default:
...
// negotiateBnaProtocol sets the Conn's bna protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateBnaProtocol(caps []p2p.Cap) {
	var highestBnaVersion uint
	for _, capability := range caps {
		if capability.Name != "bna" {
			continue
		}
		if capability.Version > highestBnaVersion && capability.Version <= c.ourHighestProtoVersion {
			highestBnaVersion = capability.Version
		}
	}
	c.negotiatedProtoVersion = highestBnaVersion
}
...
loop:
	...
	// make sure bna protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("bna protocol version must be set in Conn")
	}
	if status == nil {
...
func (s *Suite) createSendAndRecvConns(isBna66 bool) (*Conn, *Conn, error) {
	var (
		sendConn *Conn
		recvConn *Conn
		err      error
...
// readAndServe66 serves bna66 GetBlockHeaders requests while waiting
// on another message from the node.
func (c *Conn) readAndServe66(chain *Chain, timeout time.Duration) (uint64, Message) {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			resp := &bna.BlockHeadersPacket66{
				RequestId:          reqID,
				BlockHeadersPacket: bna.BlockHeadersPacket(headers),
			}
...
func (c *Conn) headersRequest(request *GetBlockHeaders, chain *Chain, isBna66 bool, reqID uint64) (BlockHeaders, error) {
	defer c.SetReadDeadline(time.Time{})
	c.SetReadDeadline(time.Now().Add(20 * time.Second))
	// if on bna66 connection, perform bna66 GetBlockHeaders request
	if isBna66 {
...
// getBlockHeaders66 executes the given `GetBlockHeaders` request over the bna66 protocol.
func getBlockHeaders66(chain *Chain, conn *Conn, request *GetBlockHeaders, id uint64) (BlockHeaders, error) {
	// write request
	packet := bna.GetBlockHeadersPacket(*request)
	req := &bna.GetBlockHeadersPacket66{
		RequestId:             id,
...
func (s *Suite) sendNextBlock(isBna66 bool) error {
	// set up sending and receiving connections
	sendConn, recvConn, err := s.createSendAndRecvConns(isBna66)
	if err != nil {
		return err
	}
	...
	if err = s.waitForBlockImport(recvConn, nextBlock, isBna66); err != nil {
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block, isBna66 bool) error {
	...
	for {
		...
		if isBna66 {
			requestID := uint64(54)
			headers, err = conn.headersRequest(req, s.chain, bna66, requestID)
		} else {
			headers, err = conn.headersRequest(req, s.chain, bna65, 0)
		}
		if err != nil {
...
func (s *Suite) oldAnnounce(isBna66 bool) error {
	sendConn, receiveConn, err := s.createSendAndRecvConns(isBna66)
	if err != nil {
		return err
	}
	defer sendConn.Close()
...
func (s *Suite) maliciousHandshakes(t *utesting.T, isBna66 bool) error {
	var (
		conn *Conn
		err  error
	)
...
func (s *Suite) hashAnnounce(isBna66 bool) error {
	// create connections
	sendConn, recvConn, err := s.createSendAndRecvConns(isBna66)
	if err != nil {
	...
	if isBna66 {
		...
		if err := sendConn.Write66(&bna.BlockHeadersPacket66{
			RequestId: id,
			BlockHeadersPacket: bna.BlockHeadersPacket{
				nextBlock.Header(),
			},
