...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "bna", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
// SetEthConfig applies bna-related command line flags to the config.
...
// RegisterEthService adds an Binacoin client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *bna.Binacoin) {
	if cfg.SyncMode == downloader.LightSync {
...
// RegisterEthStatsService configures the Binacoin Stats daemon and adds it to
// the given node.
