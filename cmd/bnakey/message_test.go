...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "bnakey-test")
	if err != nil {
	...
	// Create the key.
	generate := runBnakey(t, "generate", "--lightkdf", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runBnakey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runBnakey(t, "verifymessage", address, signature, message)
