...
import (
	...
	"github.com/binacoin-official/go-binacoin/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *bnaclient.Client {
	client, err := bnaclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to Binacoin node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, bnaclient.NewClient(client))
	if err != nil {
