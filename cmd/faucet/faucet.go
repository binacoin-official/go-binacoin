...
// faucet is an Bither faucet backed by a light client.
package main
...
var (
	...
	bnaPortFlag = flag.Int("bnaport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Binacoin protocol")
	statsFlag   = flag.String("bnastats", "", "Bnastats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Bithers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Binacoin and the faucet")
...
var (
	bither = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Bithers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *bnaPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Binacoin address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Binacoin light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Binacoin protocol stack
	client *bnaclient.Client   // Client connection to the Binacoin chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Binacoin light client protocol
	cfg := bnaconfig.Defaults
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Binacoin service: %w", err)
	}

	// Assemble the bnastats monitoring and reporting service'
	if stats != "" {
		if err := bnastats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := bnaclient.NewClient(api)
...
// close terminates the Binacoin connection and tears down the faucet.
...
// apiHandler handles requests for Bither grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, bither),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Binacoin address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/binacoin-official/go-binacoin/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), bither)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, bither)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Binacoin address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Binacoin address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Binacoin address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Binacoin address to fund on
// success.
...
// returning the username, avatar URL and Binacoin address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Binacoin address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Binacoin address,
