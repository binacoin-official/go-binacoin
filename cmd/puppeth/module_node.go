...
// nodeDockerfile is the Dockerfile required to run an Binacoin node.
var nodeDockerfile = `
FROM binacoin/client-go:latest
...
{{end}}
RUN \
  echo 'gbina --cache 512 init /genesis.json' > gbina.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.binacoin/keystore/ && cp /signer.json /root/.binacoin/keystore/' >> gbina.sh && \{{end}}
	echo $'exec gbina --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --bnastats \'{{.Bnastats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Bitherbase}}--miner.bitherbase {{.Bitherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gbina.sh

ENTRYPOINT ["/bin/sh", "gbina.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Binacoin node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.binacoin{{if .Ethashdir}}
...
// deployNode deploys a new Binacoin node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.bitherbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Bnastats":  config.bnastats,
		"Bitherbase": config.bitherbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Bnastats":   config.bnastats[:strings.Index(config.bnastats, ":")],
		"Bitherbase":  config.bitherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Bnastats username":        info.bnastats,
	}
	if info.gasTarget > 0 {
		...
		if info.bitherbase != "" {
			...
			report["Miner account"] = info.bitherbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gbina --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.binacoin"],
		ethashdir:  infos.volumes["/root/.ethash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		bnastats:   infos.envvars["STATS_NAME"],
		bitherbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
