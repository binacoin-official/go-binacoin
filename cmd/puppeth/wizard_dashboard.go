...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"bnastats", "explorer", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "bnastats":
				if infos, err := checkBnastats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have bnastats running, ask whether to make the secret public or not
	if w.conf.bnastats != "" {
		fmt.Println()
		fmt.Println("Include bnastats secret on dashboard (y/n)? (default = yes)")
