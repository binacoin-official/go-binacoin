...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for bnastats availability")
	if infos, err := checkBnastats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["bnastats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["bnastats"] = infos.Report()
		bnastats = infos.config
	}
