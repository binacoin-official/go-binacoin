...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Bithers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.bnastats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.bnastats = w.readString() + ":" + w.conf.bnastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.bnastats)
		infos.node.bnastats = w.readDefaultString(infos.node.bnastats) + ":" + w.conf.bnastats
	}
