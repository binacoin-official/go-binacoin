...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.bnastats == "" {
		log.Error("No bnastats server configured")
		return
	}
	...
	if infos.bnastats == "" {
		...
		infos.bnastats = w.readString() + ":" + w.conf.bnastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.bnastats)
		infos.bnastats = w.readDefaultString(infos.bnastats) + ":" + w.conf.bnastats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an bitherbase to mine against
			fmt.Println()
			if infos.bitherbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.bitherbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.bitherbase)
				infos.bitherbase = w.readDefaultAddress(common.HexToAddress(infos.bitherbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Binacoin node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
