...
RUN \
  echo 'gbina --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gbina --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.BnaPort}} --bootnodes {{.Bootnodes}} --bnastats \'{{.Bnastats}}\' --cache=512 --http --http.api "net,web3,bna,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gbina --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.BnaPort}} --bootnodes {{.Bootnodes}} --bnastats \'{{.Bnastats}}\' --cache=512 --http --http.api "net,web3,bna,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.BnaPort}}:{{.BnaPort}}"
            - "{{.BnaPort}}:{{.BnaPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - BNA_PORT={{.BnaPort}}
            - BNA_NAME={{.BnaName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Bnastats":  config.node.bnastats,
		"BnaPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"BnaPort":     config.node.port,
		"BnaName":     config.node.bnastats[:strings.Index(config.node.bnastats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Binacoin listener port ": strconv.Itoa(info.node.port),
		"Bnastats username":       info.node.bnastats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["BNA_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.binacoin"],
			port:     infos.portmap[infos.envvars["BNA_PORT"]+"/tcp"],
			bnastats: infos.envvars["BNA_NAME"],
		},
