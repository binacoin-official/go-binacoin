...
type config struct {
	...
	bnastats  string   // Bnastats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Binacoin services known to be running on servers
...
// converts it to an Binacoin address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by bnastats, which compares IPs textually, not by value.
