...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gbina-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "gbina-test" in runGbina.
	reexec.Register("gbina-test", func() {
...
// spawns gbina with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGbina(t *testing.T, args ...string) *testgbina {
	tt := &testgbina{}
	...
	for i, arg := range args {
		...
		case "--miner.bitherbase":
			if i < len(args)-1 {
				tt.Bitherbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
	...
	// Boot "gbina". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gbina-test", args...)
