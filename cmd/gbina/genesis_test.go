...
// Tests that initializing Gbina with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGbina(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		gbina := runGbina(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gbina.ExpectRegexp(tt.result)
		gbina.ExpectExit()
	}
}
