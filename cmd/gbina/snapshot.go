...
var (
	snapshotCommand = cli.Command{
		...
		Subcommands: []cli.Command{
			{
				Name:      "prune-state",
				Usage:     "Prune stale binacoin state data based on the snapshot",
				ArgsUsage: "<root>",
				...
				Description: `
gbina snapshot prune-state <state-root>
...
during the use of Gbina, please also specify it here for correct deletion. Otherwise
...
`,
			},
			{
				...
				Description: `
gbina snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Description: `
gbina snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Description: `
gbina snapshot traverse-rawstate <state-root>
...
`,
			...
			{
				Name:      "dump",
				Usage:     "Dump a specific block from storage (same as 'gbina dump' but using snapshots)",
				...
				Description: `
This command is semantically equivalent to 'gbina dump', but uses the snapshots
...
func pruneState(ctx *cli.Context) error {
	...
	pruner, err := pruner.NewPruner(chaindb, stack.ResolvePath(""), stack.ResolvePath(config.bna.TrieCleanCacheJournal), ctx.GlobalUint64(utils.BloomFilterSizeFlag.Name))
	if err != nil {
