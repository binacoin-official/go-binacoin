...
func NewPruner(db bnadb.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb bnadb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db bnadb.Database, trieCachePath string) error {
...
func extractGenesis(db bnadb.Database, stateBloom *stateBloom) error {
...
pruning. Remember don't start the Gbina without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "gbina snapshot prune-state --help" for more details.
`
