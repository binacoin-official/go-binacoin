...
type journalGenerator struct {
	...
	Done     bool // Whbither the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func loadAndParseJournal(db bnadb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gbina
	...
	// It can happen that Gbina crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb bnadb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Gbina was crashed without persisting journal and then
	...
	if !generator.Done {
		// Whbither or not wiping was in progress, load any generator progress too
