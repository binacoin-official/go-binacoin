...
func ReadDatabaseVersion(db bnadb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db bnadb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db bnadb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db bnadb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db bnadb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db bnadb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
