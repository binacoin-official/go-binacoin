...
func ReadSnapshotDisabled(db bnadb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db bnadb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db bnadb.KeyValueWriter) {
...
func ReadSnapshotRoot(db bnadb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db bnadb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db bnadb.KeyValueWriter) {
...
func ReadAccountSnapshot(db bnadb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db bnadb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db bnadb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db bnadb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db bnadb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db bnadb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db bnadb.Iteratee, accountHash common.Hash) bnadb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db bnadb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db bnadb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db bnadb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db bnadb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db bnadb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db bnadb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db bnadb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db bnadb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db bnadb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db bnadb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db bnadb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db bnadb.KeyValueWriter) {
