...
// genTxRing returns a block generator that sends bither in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	if !disk {
		...
		dir, err := ioutil.TempDir("", "bna-core-bench")
		if err != nil {
...
// into a database.
func makeChainForBench(db bnadb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "bna-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
