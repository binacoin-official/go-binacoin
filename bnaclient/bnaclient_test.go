...
// Verify that Client implements the binacoin interfaces.
var (
	_ = binacoin.ChainReader(&Client{})
	_ = binacoin.TransactionReader(&Client{})
	_ = binacoin.ChainStateReader(&Client{})
	_ = binacoin.ChainSyncReader(&Client{})
	_ = binacoin.ContractCaller(&Client{})
	_ = binacoin.GasEstimator(&Client{})
	_ = binacoin.GasPricer(&Client{})
	_ = binacoin.LogFilterer(&Client{})
	_ = binacoin.PendingStateReader(&Client{})
	// _ = binacoin.PendingStateEventer(&Client{})
	_ = binacoin.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  binacoin.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			binacoin.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Binacoin Service
	config := &bnaconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	bnaservice, err := bna.New(n, config)
	if err != nil {
		t.Fatalf("can't create new binacoin service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := bnaservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func TestBnaClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == binacoin.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != binacoin.NotFound {
		t.Fatal("error should be binacoin.NotFound")
	}
}
