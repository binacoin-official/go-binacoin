...
func TestGbinaClient(t *testing.T) {
...
func testGetProof(t *testing.T, client *rpc.Client) {
	ec := New(client)
	bnacl := bnaclient.NewClient(client)
	...
	// test nonce
	nonce, _ := bnacl.NonceAt(context.Background(), result.Address, nil)
	if result.Nonce != nonce {
		t.Fatalf("invalid nonce, want: %v got: %v", nonce, result.Nonce)
	}
	// test balance
	balance, _ := bnacl.BalanceAt(context.Background(), result.Address, nil)
...
func testSubscribePendingTransactions(t *testing.T, client *rpc.Client) {
	...
	// Send a transaction
	chainID, err := bnacl.ChainID(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	// Create transaction
	...
	// Send transaction
	err = bnacl.SendTransaction(context.Background(), signedTx)
	if err != nil {
		t.Fatal(err)
	}
