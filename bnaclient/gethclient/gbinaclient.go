// Copyright 2021 The go-binacoin Authors
...
// Package gbinaclient provides an RPC client for gbina-specific APIs.
package gbinaclient
...
// Client is a wrapper around rpc.Client that implements gbina-specific functionality.
//
// If you want to use the standardized Binacoin RPC functionality, use bnaclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg binacoin.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "bna_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "bna_getProof", account, keys, toBlockNumArg(blockNumber))
...
// the message call.
// Please use bnaclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg binacoin.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "bna_call", toCallArg(msg),
		toBlockNumArg(blockNumber), toOverrideMap(overrides),
	)
	return hex, err
}

// GCStats retrieves the current garbage collection stats from a gbina node.
...
// MemStats retrieves the current memory stats from a gbina node.
...
// GetNodeInfo retrieves the node info of a gbina node.
...
func (ec *Client) SubscribePendingTransactions(ctx context.Context, ch chan<- common.Hash) (*rpc.ClientSubscription, error) {
	return ec.c.BnaSubscribe(ctx, ch, "newPendingTransactions")
}
