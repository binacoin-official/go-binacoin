...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gbina.LightScryptN, Gbina.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.binacoin", new NodeConfig());
			node.start();
			...
			BinacoinClient ec = node.getBinacoinClient();
	...
	// signatures too. Regression test for go-binacoin issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gbina-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/binacoin/gbinatest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gbina and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.binacoin", "github.com/binacoin-official/go-binacoin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gbina.aar"), "gbina.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "binacoin", "gbinatest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.binacoin.gbinatest"
