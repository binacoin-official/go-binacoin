...
// Package bnastats implements the network stats reporting service.
package bnastats
...
import (
	...
	ethproto "github.com/binacoin-official/go-binacoin/bna/protocols/bna"
...
// backend encompasses the bare-minimum functionality needed for bnastats reporting
...
// Service implements an Binacoin netstats reporting daemon that pushes local
...
// parseBnastatsURL parses the netstats connection url.
...
func parseBnastatsURL(url string) (parts []string, err error) {
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	parts, err := parseBnastatsURL(url)
	if err != nil {
		return err
	}
	bnastats := &Service{
	...
	node.RegisterLifecycle(bnastats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["bna"]; info != nil {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the bnastats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to bnastats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to bnastats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to bnastats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to bnastats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to bnastats")
