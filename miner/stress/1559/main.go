...
func main() {
	...
	for i := 0; i < 4; i++ {
		...
		stack, bnaBackend, err := makeMiner(genesis)
		if err != nil {
			panic(err)
		}
		defer stack.Close()
...
func makeMiner(genesis *core.Genesis) (*node.Node, *bna.Binacoin, error) {
	// Define the basic configurations for the Binacoin node
	...
	// Create the node and configure a full Binacoin node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, nil, err
	}
	bnaBackend, err := bna.New(stack, &bnaconfig.Config{
		Genesis:         genesis,
		...
		GPO:             bnaconfig.Defaults.GPO,
		Ethash:          bnaconfig.Defaults.Ethash,
		Miner: miner.Config{
