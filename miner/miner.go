...
// Package miner implements Binacoin block creation and mining.
package miner
...
type Config struct {
	Bitherbase  common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
...
func New(bna Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		bna:     bna,
		mux:     mux,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, bna, mux, isLocalBlock, true),
	}
	go miner.update()
	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.FailedEvent:
				canStart = true
				if shouldStart {
					miner.SetBitherbase(miner.coinbase)
					miner.worker.start()
				}
...
func (miner *Miner) SetBitherbase(addr common.Address) {
	miner.coinbase = addr
	miner.worker.setBitherbase(addr)
}
